function caesarEncrypt(text, key) {
  ...
  function vigenereEncrypt(text, key) {
    ...
    for (let i = 0; i < text.length; i++) {
      ...
      if (char.match(/[a-z]/i)) {
        ...
        const encryptedChar = String.fromCharCode(((char.charCodeAt(0) - base + k) % 26) + base);
        steps.push(`${char} → ${encryptedChar} (${char} + ${key[j % key.length]})`);
  ...
  function vigenereDecrypt(text, key) {
    ...
    for (let i = 0; i < text.length; i++) {
      ...
      if (char.match(/[a-z]/i)) {
        const base = char === char.toUpperCase() ? 65 : 97;
        const k = key[j % key.length].charCodeAt(0) - 65;
        const decryptedChar = String.fromCharCode(((char.charCodeAt(0) - base - k + 26) % 26) + base);
        steps.push(`${char} → ${decryptedChar} (${char} - ${key[j % key.length]})`);
  ...
  function formatPlayfairMessage(msg) {
    msg = msg.toUpperCase().replace(/J/g, 'I').replace(/[^A-Z]/g, '');
    let result = '';
  ...
  function playfairEncrypt(message, key) {
    ...
    const encrypted = pairs.map(pair => {
  ...
  function playfairDecrypt(message, key) {
    const steps = [];
    const matrix = createPlayfairMatrix(key);
    const pairs = message.toUpperCase().replace(/[^A-Z]/g, '').match(/.{1,2}/g);
    const decrypted = pairs.map(pair => {
      const [row1, col1] = findPosition(matrix, pair[0]);
      const [row2, col2] = findPosition(matrix, pair[1]);
